...
router.post('/',function(req,res){
	...
	var login_id = req.body.givenlogin_id;
	...
	else
	{
		...
		pool.getConnection(function(err,conn){
			...
			var exec = conn.query('select * from users where login_id=?',login_id,function(err,result){
				if(err){res.send({tokenValid:true,payload:{available:true,status:0}});}
				else{
					if(((my_auth != 100) && (my_auth != 0)) && ( result[0].auth !=2) ){res.send({available:true,status:3});  }
					else if( result[0].auth == 0){res.send({tokenValid:true,payload:{available:true,status:3}});}
					else{
						data = [given_auth,login_id];
